...
async function init() {
    try {
        ...
        const caihong = await fetch(
            `http://api.tianapi.com/txapi/caihongpi/index?key=${tianXingKey}`
        )
        const newslist = await caihong.json();
        const {content} = newslist.newslist[0];
        ...
        // 用邮件模版生成字符串
        const htmlStr = emailHtml(weatherData, lifeData, word, imgurl, lovingDays, content);
